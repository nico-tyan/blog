 ...
 */
public class CustomOneStepProcessor2 implements PageProcessor {
	private String domain = null;
	private String schedulerFilePath = null;
	private String pipelineFilePath = null;
	private Integer depthStep = 1;

	public CustomOneStepProcessor2(String domain) {
		this.domain = domain;
	}

	public CustomOneStepProcessor2(String domain, Integer depthStep) {
		this.domain = domain;
...
//		driver.quit();// 退出浏览器
		...
		System.setProperty("selenuim_config", CustomOneStepProcessor2.class.getClassLoader().getResource("config.ini").getPath());
		Spider.create(new CustomOneStepProcessor2("zt.voc.com.cn")).addUrl(url)
				// .addUrl("http://www.xxcb.cn/event/caijing/")
				//.setScheduler(new CustomFileCacheQueueScheduler("D:/webmagic/url1234/"))
